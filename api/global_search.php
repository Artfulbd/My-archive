<?php
    ...
    if($link == null){
    ...
    else if(!$kit->_validate($data, $kit->get_global_search_list())){
        $conObg->detach();
        ...
        if($data->global){
            ...
            $qry = "SELECT ct.`content_id`, type.type, topic.`topic`, ct.`title`, ct.`content_text` FROM `content` ct 
            join topic on ct.topic_id = topic.topic_id 
            join type on topic.type_id = type.type_id 
            WHERE ct.title LIKE '%$data->global%' or ct.content_text LIKE '%$data->global%' or type.type LIKE '%$data->global%' or topic.topic LIKE '%$data->global%' limit $data->limit ;";
            try {
